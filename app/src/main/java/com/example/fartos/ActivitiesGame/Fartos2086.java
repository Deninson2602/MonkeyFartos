...
import com.example.fartos.RecyclerCartas.Cartas_recycler;
import com.example.fartos.RecyclerTablero.Casilla;
import com.example.fartos.RecyclerTablero.Casilla_recycler;
import com.example.fartos.Taulell;
...
public class Fartos2086 extends Activity {
    ...
    Casilla_recycler adapter = new Casilla_recycler(casillas, this);
    Cartas_recycler adapterCartas = new Cartas_recycler(cartas);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fartos2086);
        ...
        adapter.setOnItemClick(new Casilla_recycler.OnItemClick() {
            @Override
        ...
        next.performClick();
        */
        adapterCartas.setOnItemClick(new Cartas_recycler.OnItemClick() {
            @Override
            public void getPosition(int pos) {
                ...
                switch(jugadorActual.getMa().get(pos).getEfecte()) {
                    case HUNDIMIENTO:
                    {
                        if(jugadorSelecionado == null){
                            Toast.makeText(Fartos2086.this, "seleciona un jugador", Toast.LENGTH_SHORT).show();
                        }else {
    ...
    public void ganador(){
        if(jugadors.size() == 1){
            Toast.makeText(Fartos2086.this, "ganador es " + jugadors.get(0).getNom(), Toast.LENGTH_SHORT).show();
        }else{
            for (Jugador jugador: jugadors){
                if(jugador.getPosition() == casillas.size() - 1){
                    Toast.makeText(Fartos2086.this, "ganador es " + jugador.getNom(), Toast.LENGTH_SHORT).show();
                    break;
                }
            }
        }
